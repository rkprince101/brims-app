...
import { Plus, Search, ClipboardList, MoreHorizontal, X, CheckCircle2 } from "lucide-react";
...
export default function WorkOrders() {
  const { workOrders, loading, addWorkOrder, closeWorkOrder } = useWorkOrders();
  const { veps } = useVEPs();
  ...
  const closableWorkOrders = workOrders.filter(
    (wo) => wo.status === "PENDING" || wo.status === "ACCEPTED"
  );
  ...
  const handleCloseWO = async () => {
    ...
    await closeWorkOrder(selectedWO.id, closingRemark);
  ...
  return (
    <div className="max-w-6xl mx-auto px-8 py-10 animate-fade-in">
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {filteredOrders.map((wo) => {
                const isClosable = wo.status === "PENDING" || wo.status === "ACCEPTED";
                return (
                  <tr
                    key={wo.id}
                    ...
                    <td className="py-3 px-4 text-right">
                      {isClosable && (
                        <div className="relative inline-block">
                          <button
                            onClick={(e) => { e.stopPropagation(); setActionMenu(actionMenu === wo.id ? null : wo.id); }}
                            className="p-1.5 rounded hover:bg-border text-text-muted hover:text-text-primary transition-colors"
                          >
                            <MoreHorizontal size={16} />
                          </button>
                          {actionMenu === wo.id && (
                            <div className="absolute right-0 top-full mt-1 bg-bg-card border border-border rounded-lg shadow-lg py-1 z-20 min-w-[160px]">
                              <button
                                onClick={() => handleOpenCloseModal(wo)}
                                className="w-full text-left px-3 py-2 text-xs text-danger hover:bg-danger-bg flex items-center gap-2 transition-colors"
                              >
      ...
      {closeModalOpen && selectedWO && (
        <Modal title="Close Work Order (RCC)" onClose={() => setCloseModalOpen(false)}>
          <div className="space-y-4">
            <div className="p-3 rounded-lg bg-warning-bg border border-warning/20 text-sm">
              <p className="text-warning font-medium mb-1">Work Order: {selectedWO.workOrderNumber}</p>
              <p className="text-text-secondary text-xs">VEP: {selectedWO.vep?.registrationNumber} &mdash; {selectedWO.workType.replace(/_/g, " ")}</p>
            </div>
            <div>
            ...
            <p className="text-xs text-text-muted italic">This will close the work order with status &ldquo;RCC Closed&rdquo;. No job card will be created.</p>
            <div className="flex justify-end gap-2 pt-2 border-t border-border">
              ...
              >
                {closingLoading ? "Closing..." : <span className="flex items-center gap-1"><CheckCircle2 size={14} /> Close Work Order</span>}
              </button>
            </div>
          </div>
        </Modal>
