...
export function useVEPs() {
  ...
  const bulkImportVEPs = async (veps) => {
    const data = await fetchApi("/api/vep", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ veps }),
    });
    setVeps((prev) => [...data.created, ...prev]);
    return data;
  };

  return { veps, loading, refetch: fetchVeps, addVEP, updateVEP, deleteVEP, bulkImportVEPs };
}
...
export function useWorkOrders() {
  ...
  const closeWorkOrder = async (id, closingRemark) => {
    const data = await fetchApi(`/api/work-orders/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return { workOrders, loading, refetch: fetchWOs, addWorkOrder, updateWorkOrder, deleteWorkOrder, closeWorkOrder };
}
...
export function useJobCards() {
  ...
  const closeJobCard = async (id, closingRemark) => {
    const data = await fetchApi(`/api/job-cards/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        status: "CLOSED_BY_RCC",
        closingRemark,
  ...
  const linkJobCards = async (childId, parentJobCardId) => {
    const data = await fetchApi(`/api/job-cards/${childId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ relatedJobCardId: parentJobCardId }),
    });
    setJobCards((prev) => prev.map((j) => (j.id === childId ? data : j)));
    return data;
  };

  const unlinkJobCards = async (childId) => {
    const data = await fetchApi(`/api/job-cards/${childId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ relatedJobCardId: null }),
    });
    setJobCards((prev) => prev.map((j) => (j.id === childId ? data : j)));
    return data;
  };

  return { jobCards, loading, refetch: fetchJCs, addJobCard, updateJobCard, closeJobCard, linkJobCards, unlinkJobCards };
}
...
export function useUnits() {
  ...
  const fetchUnits = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchApi("/api/units");
      setUnits(data || []);
    } catch (e) { console.error(e); }
    setLoading(false);
  }, []);
  ...
  const updateUnit = async (id, updates) => {
    const data = await fetchApi(`/api/units/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(updates),
    });
  ...
  return { units, loading, refetch: fetchUnits, addUnit, updateUnit, deleteUnit };
}
